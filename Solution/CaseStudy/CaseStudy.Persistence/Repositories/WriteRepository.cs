...
namespace CaseStudy.Persistence.Repositories
{
    ...
    {
        readonly private CaseStudyDbContext _context;
        ...
        {
            T model = await Table.FirstOrDefaultAsync(data => data.Id == Guid.Parse(id)) ?? throw new InvalidOperationException("Null"); ;
            return Remove(model);
        }
